...
test("Büyük değeri olan argümanlar aldığında doğru dizini dönmeli", () => {
  expect(fibonacci(1, 1000000)).toEqual([
...
test("Başlangıç ve bitiş değerine uygun fibonacci dizinini getirmeli", () => {
  expect(fibonacci(3, 8)).toEqual([3, 5, 8]);
});
...
test("Dönülen dizin array olmalı", () => {
  expect(Array.isArray(fibonacci(10, 20))).toBeTruthy;
});
...
test("Decimal argüman aldığında dönüş yapmalı", () => {
  expect(fibonacci(4.3, 18.8)).toEqual([5, 8, 13]);
});
...
test("Argümanlar sayı değilse hata yollamalı", () => {
  ...
  expect(() =>
    fibonacci([1, 2, 3], { name: "leonardo", surname: "fibonacci" })
  ).toThrow();
});
